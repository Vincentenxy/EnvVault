...
v11 增量（Secret 批量创建,跨 env 同 key 多 value,显式指定每个 env 的目标 folder）:`POST /api/v1/secret/create` 是单条创建。当用户要在一个 project 下多个 env(dev/test/sim/prod)里给同一个 key(如 `DATABASE_URL`)设置不同 value 时,需要 N 次 round-trip,N 次权限校验 + N 条 audit。本轮加 `POST /api/v1/secrets/batchCreate`(注意:复数 `secrets`,与单条 `/secret/*` 区分,**单独路由组**,不混在 `/secret/*` 下),接收 `secretList`(`[{key, comment?, dev: {folderId, value}, test: {folderId, value}, sim: {folderId, value}, prod: {folderId, value}}]`),**每个 env 字段客户端显式指定目标 folderId + value**,**单事务**完成 N 条 INSERT + 1 条 batch audit,全成功或全 rollback。
- 端点: `POST /api/v1/secrets/batchCreate`(独立路由组 `/secrets`,不与单条 `/secret/*` 混)。
- 入参: `{secretList: [{key, comment?, dev?: {folderId, value}, test?: {...}, sim?: {...}, prod?: {...}}]}`。要求 `secretList` 非空,每条 `key` 通过 `^[A-Z][A-Z0-9_]*$`,每条至少要指定一个 env(4 个中至少 1 个非空),每个 env 字段的 `folderId` 必填。
...
- 编排:`SecretService.BatchCreate` 1) 防御性二次校验 `user.UserId` 非空 / `secretList` 非空 / 每条 `key` 合法 / 每条 `Envs` 非空 / 每个 `env.FolderId` 非空;2) 展开 `secretList × Envs` 为 (envCode, folderId, value, key, comment) target 序列;3) 对每个 target 单独 `secret:create` 权限 check(v7 cascade narrowing),任一拒绝即整批失败、err 透传;4) 构造 `[]store.BatchCreateSecretItem` 调 `r.BatchCreateSecrets`;5) commit 后逐条 `cacheUpsert` 到 Redis SecretCache。
...
- 硬编码 4 个 env 字段:`dev` / `test` / `sim` / `prod` 是项目下标准 4 env;非项目下 4 个标准 env 的项目暂不支持(后续可按需扩展为 map form)。env code 顺序固定为 `dev → test → sim → prod`,用于 audit 列举。
- 不支持:`folderParentId` 嵌套 / `includeSubfolders` 嵌套(与单条 CreateSecret 保持一致);env 字段里 folderId 为空字符串(整条 item 拒绝);跨 project 批量创建(每条 item 各自的 folderId 必须在 caller 有 secret:create 权限,无 project 级批量语义)。
...
详细见 `design/DESIGN.md`「Secret 批量创建（v11）」节。
