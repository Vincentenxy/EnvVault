...
type ResourceRepository interface {
	...
	// keys 必须非空(本接口 caller 保证),SQL 用 `s.key = any($5::text[])` 精确匹配;
	...
	ListSecretsByProjectFolderKey(ctx context.Context, callerUserId, action, projectId, folderCode string, keys []string, envCodes []string) ([]domain.Secret, [][]byte, error)
