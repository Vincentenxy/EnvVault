...
每个 list 方法的 WHERE 末尾追加一段 narrowing 谓词，核心是一个 CTE 把「caller 持有 `permissionCode`
...
| --- | --- | --- |
| `ListOrganizations` | `org:read` | `t.id in (select scope_id from user_read_scopes where scope_type = 'organization')` |
| `ListProjects` | `project:read` | `t.id in (… 'project')` ∪ `t.org_id in (… 'organization')` |
| `ListEnvironments` | `env:read` | `t.id in (… 'environment')` ∪ `t.project_id in (… 'project')` ∪ `p.org_id in (… 'organization')`(需 join projects) |
