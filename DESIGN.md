...
- `single`：单节点模式，开发环境默认使用。
- `cluster`：集群模式，`addrs` 配置多个 Redis Cluster 节点地址。

Redis 用于 Secret 查询缓存。应用启动时会从 PostgreSQL 加载 active secrets 到 Redis，保存内容包含 `key` 和加密后的 `value_ciphertext`，不会保存明文 value。Secret 创建、更新、删除后会同步更新 Redis。

## 日志与链路追踪

HTTP 请求进入后会读取请求头中的 request id，并写回响应头。默认请求头名称为 `x-request-id`，可通过配置修改：
...
如果请求未携带 request id，服务端会自动生成一个 UUID。
...
HTTP 访问日志会打印请求路径、路由、状态码、客户端 IP 和耗时，类似 Gin access log。业务日志会在各接口入口打印方法名和关键参数。

脱敏规则：

- 字段名包含 `value`、`password`、`secret`、`token`、`cookie`、`jwt` 时，日志值统一打印为 `***`。
- Secret 的 `key` 可以打印。
- Secret 的 `value` 不允许打印明文。

## 本地运行

先启动本地 PostgreSQL 和 Redis：
...
- `ENVVAULT_HTTP_REQUEST_ID_HEADER`：链路追踪请求头名称，默认 `x-request-id`
...
- `ENVVAULT_REDIS_ENABLED`：是否启用 Redis，默认 `true`
...
Secret key 搜索优先使用 Redis 缓存；Redis 不可用或查询失败时回退到 PostgreSQL。Redis 中的 `value_ciphertext` 仅保存密文，用于后续 value 搜索方案预留，不保存明文。
